...
def aux(filename):
	try:
		...
		label = KMeans(n_clusters=len(pivot_df.columns), random_state=1993).fit_predict(pivot_df)
		pivot_df['label'] = label
		pivot_df.to_csv(os.path.join(DEST_DIR_CNT, filename))
	except Exception as e:
		print("aux error: %s" % e)
...
if __name__ == '__main__':
	try:
		...
		# rss_center_kmeans(2, k0=0, k1=2)
		# count_kmeans('000000000000.csv')
		# count_sample_kmeans(2, k=0)
		# count_center_kmeans(2, k0=0, k1=2)
		aux('000000000000.csv')
	except Exception as e:
		raise e