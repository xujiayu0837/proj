...
SRC_DIR = '/usr/local/data/201710'
DEST_DIR = '/usr/local/data/clean_data'
...
def filter_invalid_data(data):
	try:
		data = data[(data['rss'].astype(str).str.len() == 3) | (data['rss'].astype(str).str.endswith('.0'))]
		data = data[(data['ts'].astype(str).str.len() == 10) | (data['ts'].astype(str).str.endswith('.0'))]
		return data
	except Exception as e:
